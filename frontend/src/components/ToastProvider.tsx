...
type Toast = { id: number; message: string; type: ToastType; hiding?: boolean }
...
export const ToastProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const push = useCallback((message: string, type: ToastType, durationMs = 3000) => {
    const nextId = id + 1
    setId(nextId)
      ...
      <div className="fixed bottom-4 right-4 z-[1000] space-y-2">
        {toasts.map((t) => (
          <div
            key={t.id}
            className={
              'min-w-[220px] max-w-[360px] rounded-lg shadow-xl px-4 py-3 text-sm text-white ring-1 ring-black/5 transition-all duration-200 transform ' +
